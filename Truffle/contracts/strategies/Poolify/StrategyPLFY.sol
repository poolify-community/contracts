...
import "../../protocol/PoolifyChef.sol";
...
contract StrategyPLFY is StratManagerPLFY, FeeManagerPLFY {
    ...
    function deposit() public whenNotPaused {
        ...
        PoolifyChef(masterChef).deposit(0,wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            PoolifyChef(masterChef).withdraw(0,_amount.sub(wantBal));
    ...
    function harvest() public whenNotPaused {
        ...
        uint256 wantBal = PoolifyChef(masterChef).pendingPoolify(0,address(this));
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,) = PoolifyChef(masterChef).userInfo(0, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        PoolifyChef(masterChef).emergencyWithdraw(0);
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(address(masterChef), type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(address(masterChef), 0);
    }
}
